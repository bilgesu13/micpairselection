...
for r in range(num_runs):
    ...
    x_TD = np.load(f"x_TD_homula_FINAL__snr0dB_run{r}.npy")
    ...
    mic_coords = np.load('mic_coord_homula.npy')
    ...
    source_coords =  [2.28, 0.96, 1.2] #### fixed in HOMULARIR dataset
    ...
    for runs in range(num_runs):
        ...
        # np.random.seed(6) 
        # ### for 0 dB ###
        # #[24, 65, 113, 261, 188, 181] 8.9 m         np.random.seed(10) 
        # #[39, 38, 183, 13, 90, 165] 0.99 m np.random.seed(9)
        # #[96, 246, 141, 244, 222, 89] 7.4 m     np.random.seed(8)
        # #[254, 57, 150, 66, 114, 280] 1.74  np.random.seed(7)
        # # [27, 136, 213, 184, 93, 289] 0.06   np.random.seed(6)
        # ### for 6 dB ###
        # #[24, 65, 113, 261, 188, 181] 0.59 m         np.random.seed(10) 
        # #[39, 38, 183, 13, 90, 165]  0.64 m np.random.seed(9)
        # #[96, 246, 141, 244, 222, 89]  2.9 m     np.random.seed(8)
        # #[254, 57, 150, 66, 114, 280] 1.59 m  np.random.seed(7)## put this in figure
        # # [27, 136, 213, 184, 93, 289] 0.06 m   np.random.seed(6)
        # ### for 12 dB ###
        # #[24, 65, 113, 261, 188, 181] 0.59 m         np.random.seed(10) 
        # #[39, 38, 183, 13, 90, 165]  0.64 m np.random.seed(9)
        # #[96, 246, 141, 244, 222, 89]  1.25 m     np.random.seed(8)
        # #[254, 57, 150, 66, 114, 280] 1.34 m  np.random.seed(7)
        # # [27, 136, 213, 184, 93, 289] 0.112 m   np.random.seed(6)
        ...
        P = len(above_threshold_indices)
        SRP_conv_graph = SRP_conv_graph/P  # normalize for better visualization
        ...
        plt.scatter(estim_DOAvec[0,0,0]/stepsize,estim_DOAvec[0,0,1]/stepsize,marker = 'o', edgecolors='b', facecolors='none', linewidths=2, s=200)
...
print("=====================================")
print(f"Mean error over 50 runs:   {mean_err:.3f} m")
print(f"Median error over 50 runs: {median_err:.3f} m")


# Save errors for later analysis
#np.save("loc_errors_sparsity2_100runs_homula_0dB.npy", err_euclidean_all)
#np.save("loc_errors_sparsity2_50runs_homula_0dB_cropped.npy", err_euclidean_all)


#print("\nSaved all 50 errors to loc_errors_sparsity2_50runs_homula_0dB_cropped.npy")



### also ran random microphone pair selection for 50 runs and saved errors to loc_errors_random_50runs_homula_0dB.npy ### 6dB,12 dB



np.save("loc_errors_FINAL__homula_sparsity1_0dB.npy", err_euclidean_all)
