### compute SRP for different source locations
### change SNR
...
source_coords = np.load('200_new_realistic_source_coords.npy')  
...
for runs in range(num_runs):
    ...
    # # Create the room with materials
    # room = pra.ShoeBox(
    ...
    ### for reverb time 0.74s absorb_coeff = 0.4, scattering_coeff = 0.5 change reverb here ###

    absorption_coeff = 0.4 #0.4 #0.7 #0.5 #0.32 #0.26 
    scattering_coeff = 0.5 #0.5 #0.7 #0.6 #0.55 #0.4 
    ...
    for i in range(len(microphone_combinations)):
        #results[i] = sparsity_measure(cross_correlations[i,:])  #sparsity measure 1 
        results[i] = kurtosis(cross_correlations[i,:]) #sparsity measure 2 
    ...
    # Plot points above the threshold with a different color
    #plt.scatter(above_threshold_x, above_threshold_results, s=10, color='r', label=f'Above threshold')

    # Annotate nodes above the threshold
    # for i in range(len(above_threshold_x)):
    #     plt.annotate(f'{above_threshold_x[i]}', (above_threshold_x[i], above_threshold_results[i]),
    #                 textcoords="offset points", xytext=(0, 5), ha='center', fontsize=8, color='r')

    # # Labeling the axes and title
    # plt.xlabel('Microphone Pair Index', fontsize=12)
    # plt.ylabel('Sparseness', fontsize=12)
    # plt.title('Sparseness Values with local threshold', fontsize=14)
    # plt.legend()
    # plt.grid(True)
    # plt.show()
...
np.save('measure2200newsource_realistic_snr24', err_euclidean_all)


#np.save('measure1200newsource_realistic_rev074', err_euclidean_all)

#np.save('measure1200newsource_realistic6dB_8pairs', err_euclidean_all)

#np.save('measure2200newsource_realistic_snr24', err_euclidean_all)

#np.save('measure2200newsource_0_5_resolution_snr0', err_euclidean_all)

#np.save('measure2200newsource_0_5_resolution_snr0_rev074_2pairs', err_euclidean_all)



#np.save('total_error_different_locations.npy', total_error) 
#np.save('error_euclidean_all_for_different_locations_measure1_M16', err_euclidean_all)
